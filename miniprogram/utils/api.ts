import { UserEventStatusResponse, RegistrationParams, RegistrationResult } from '../types/types';

export const API_CONFIG = {
  baseUrl: 'https://yesapi.txrczx.cn/',
  appKey: '601403AC619E9330F004A888A5F12DE2',
  sign: 'E336AE08D2B4363B1309B918719CEA58'
};
...
export const api = {
  ...
  getWeixinInfo: (code: string): Promise<any> => {
    ...
    return new Promise((resolve, reject) => {
      wx.request({
        url: API_CONFIG.baseUrl + `?s=App.Weixin.GetWeixinInfoMini&return_data=0&code=${code}&app_key=${API_CONFIG.appKey}&sign=${LOGIN_SIGN}`,
        method: 'GET',
        success: (res: WechatMiniprogram.RequestSuccessCallbackResult) => {
  ...
  submitRegistration: (params: RegistrationParams): Promise<RegistrationResult> => {
    ...
    return new Promise((resolve, reject) => {
      wx.request({
        url: API_CONFIG.baseUrl + `?s=SVIP.Sxuwenkai357_MyApi.ASubmitRegistration&app_key=${API_CONFIG.appKey}&sign=${SIGN}`,
        method: 'POST',
        header: {
